...
export default function ProductDetail({ params }) {
  ...
  const handleCrop = useCallback(async () => {
    ...
    const croppedImageUrl = await getCroppedImg(image, croppedAreaPixels);
    setCroppedImage(croppedImageUrl); // Set cropped image in state
    setImage(null); // Clear the original image
  }, [image, croppedAreaPixels]);
  ...
  const handleSubmit = async () => {
    ...
    if (croppedImage) {
      // Upload the cropped image to Firebase Storage only on submit
      const response = await fetch(croppedImage);
      const blob = await response.blob();
      const imageRef = ref(
        storage,
        `product-images/${product._id}-${Date.now()}`
      );
      await uploadBytes(imageRef, blob);
      const newImage = await getDownloadURL(imageRef);
      formData.append("image", newImage); // Add the uploaded image URL to form data
    } else {
      formData.append("image", productImage); // If no new image, use the current image URL
    }
    ...
    try {
      if (role === "admin") {
        ...
        await axios.post("/api/products/review", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
